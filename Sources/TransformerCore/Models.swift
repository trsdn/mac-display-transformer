...
public enum TransformGeometry {
    ...
    /// Describes a layer whose anchor and position are both at the origin.
    /// The returned affine performs orientation, mirroring, aspect fit, and
    /// centering exactly once; callers must use fill gravity for its content.
    ...
    /// Builds clockwise rotation and mirrors in post-rotation axes, normalized
    /// so that the transformed image starts at the origin.
    ...
    /// Builds the complete Core Image transform: orientation, aspect fit, and
    /// centering in the requested target bounds.
...
public struct PersistentDisplayIdentity: Codable, Equatable, Hashable, Sendable {
    ...
    /// Rotation-independent long edge of the display mode in pixels.
    public let nativeLongEdge: Int
    /// Rotation-independent short edge of the display mode in pixels.
...
public enum DisplayIdentityMatcher {
    /// Returns an index only when the strongest identity match is unique.
    ...
    private static func score(
        ...
        if stored.serialNumber != nil, candidate.serialNumber == nil {
            // Never downgrade a previously stable hardware identity.
            return nil
        }
        ...
        if storedHardware != nil, storedHardware == candidateHardware {
            return fallbackMatches ? 220 : 180
        }
...
public struct PresetSlot: Codable, Equatable, Sendable {
    ...
    public init(name: String, configuration: PresetConfiguration = .init()) {
        self.name = name
